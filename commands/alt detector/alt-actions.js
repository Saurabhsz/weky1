...
module.exports.run = async (client, message, args, utils, data) => {
  ...
  if (!args[0]) return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.fail} ${language.aactionNotValidChoice.replace("{allChoices}", choices.join(", "))}`).setFooter('https://pogy.xyz').setTimestamp().setColor('RED'));
  ...
  await alt.findOne({ guildID: message.guild.id }, async (err, db) => {
    if (!db) {
      ...
      return message.channel.send(new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${message.client.emoji.success} ${language.aactionSuccess.replace("{action}", args[0])}`).setFooter('https://pogy.xyz').setTimestamp().setColor('RED'));
    }
    await db.updateOne({
      altAction: args[0].toLowerCase()
    })
